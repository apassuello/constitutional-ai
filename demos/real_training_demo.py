...
def evaluate_model_responses(
    ...
    with torch.no_grad():
        for prompt in tqdm(prompts, desc=desc):
            # Generate response
            response = generate_text(model, tokenizer, prompt, generation_config, device=device)
...
def visualize_comparison(
    ...
    if "epoch_rewards" in training_history and training_history["epoch_rewards"]:
        ...
        ax4.plot(
            epochs,
            training_history["epoch_rewards"],
            marker="s",
            linewidth=2,
            markersize=8,
            color="#51cf66",
    ...
    models = ["Baseline", "Fine-tuned"]
    flagged_counts = [baseline_metrics["num_flagged"], finetuned_metrics["num_flagged"]]
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="gpt2", help="Model to use (gpt2, distilgpt2, etc.)"
    )
    parser.add_argument("--num_epochs", type=int, default=2, help="Number of training epochs")
    parser.add_argument("--num_prompts", type=int, default=15, help="Number of training prompts")
    parser.add_argument(
    ...
    parser.add_argument(
        "--quick_demo", action="store_true", help="Run quick demo (1 epoch, 5 prompts)"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output/constitutional_real_training",
    ...
    try:
        model, tokenizer = load_model(
            model_name=args.model, device=device, load_in_8bit=args.load_in_8bit
        )
    ...
    generation_config = GenerationConfig(
        max_length=150, temperature=0.7, do_sample=True, top_p=0.9, top_k=50
    )
    ...
    visualize_comparison(
        baseline_metrics, finetuned_metrics, training_result["training_history"], viz_path
    )
